...
packages <- c("tidyverse", "igraph","readr","readxl", "broom", "zoo", "stringr","foreach", "doMC",  "xtable", "geomnet", "ggnetwork", "rlang", "animation", "ggridges", "poweRlaw", "Rtsne", "caret", "ranger", "xtable", "broom", "modelr")
...
predictions %>%
  summarize(MSE = mean((Link.Limit - .fitted) ^ 2),
            MSEIntercept = mean((Link.Limit - mean(Link.Limit))^2))

postResample(predictions$.fitted, predictions$Link.Limit)
...
SummaryofGC %>% 
 ...
 NodeCollapse2 %>%
   ...
   geom_line(data = NodeCollapse %>% 
               filter(alpha == "Linear Model_Limits",is.finite(sd)), 
             aes(x= (PercNum), y = mean), colour = "blue" ) +
  scale_x_continuous(labels=scales::percent) + 
...
TSNEdf <- AllRemovedEdges %>%
 # filter(alpha %in% c("Real_Limits", "Linear_Model_Limits")) %>%
...
tsne_corplot %>%
  ...
  geom_point(aes(x=x1, y=y1, color= alpha))+
    geom_point(data = filter(tsne_corplot, alpha2 == "Real Limits"), aes(x = x1, y = y1), colour = "dodgerblue2") +
  geom_point(data = filter(tsne_corplot, alpha2 == "Linear Model Limits"), aes(x = x1, y = y1), colour = "black") +
AlphaGrad +
...
alpha %in% c("alpha_value_100" , "alpha_value_500","Infinite_Limits", "Linear_Model_Limits" )
...
ConfMat$table %>%
  xtable(.,  caption = "Confusion matrix of random forest", label = "tab:ConfusionForest") %>%
...
BasePlot <- AccuracyAnalysis %>% 
  ggplot(aes(x= as.factor(alpha), fill = alpha)) +
  theme(legend.position = "none",  
        axis.text.x = element_text(angle = 45, hjust = 1)) +
   scale_fill_gradientn(colors = c("red",   "limegreen"),
                          #midpoint = log(median(NodeCollapse2$alpha)),
...
BasePlot + geom_boxplot(aes(y = RMSE)) +
...
BasePlotGC + geom_boxplot(aes(y = RMSE)) +
...
Mod1 <- lm(Link.Limit ~Voltage  + absPF, data = edges2[trainset,])
summary(Mod1)
Preds <- predict(Mod1, edges2[-trainset,])

postResample(Preds, edges2$Link.Limit[-trainset])

Predsdf <- data_frame(Preds, 
                      Obs = edges2$LLbin2[-trainset],
...
ElectCentr2 <- ElectCentr %>%
  ...
  mutate(metric = "MyMetric",
         ...
         Scale_Fract = ifelse(Scale_Fract<0, 0, Scale_Fract))  %>% #prevents 0 from being negative
#Alpha max 0.95 alpha min 0.05
mutate(alpha_high = (200-1.05)*Scale_Fract + 1.05,
       alpha_low = (200-1.05)*(1-Scale_Fract) + 1.05,
       Edge_Centrality_high_95 = abs(PowerFlow*alpha_high),
       Edge_Centrality_low_95 = abs(PowerFlow*alpha_low))  %>%
   filter(Link.Limit!=1e5, Voltage!=0
         )
...
postResample(ElectCentr2$Edge_Centrality_high_95, ElectCentr2$Link.Limit)


trainset <- createDataPartition(ElectCentr2$Link.Limit p = .75, list = FALSE)[,1] #trainset as vector


Mod1 <- lm(Link.Limit ~Voltage  + absPF, data = ElectCentr2[trainset,])
summary(Mod1)

Preds <- predict(Mod1, ElectCentr2[-trainset,])

postResample(Preds, ElectCentr2$Link.Limit[-trainset])

Predsdf <- data_frame(Preds, 
                      Obs = ElectCentr2$Link.Limit[-trainset],
                      PowerFlow = ElectCentr2$PowerFlow[-trainset],
                      Voltage = ElectCentr2$Voltage[-trainset],) %>%
  mutate(error = Obs-Preds,
         abserror = abs(error),
         PercError = error/Obs,
         absPercError = abserror/Obs,
...
Predsdf %>%
ggplot(aes(x= Obs, y=(PercError), colour = as.factor(Voltage))) +geom_point()

Predsdf %>%
ggplot(aes(x= log10(abserror), y=PercError, colour = as.factor(Voltage))) +geom_point()
...
NodeCollapse3 <- NodeCollapse2 %>%
 ...
 ModLim <- lm(PercNum~ log(alpha) + log(alpha2), data = MaxLim)
 summary(ModLim)
...
ggplot(MaxLim, aes(x = log(alpha), y = PercNum)) +geom_point() +

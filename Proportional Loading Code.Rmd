...
test %>%  CleanNames(., "alpha", AlphaOrder2) %>%
...
TSNEdf <- AllRemovedEdges %>%
  group_by(Sim, alpha) %>%
  mutate(ntile = ntile(RoundRemoved, 20)) %>%
  ungroup %>%
  spread(key = Edges, value = ntile, fill = 0)
...
AlphavsCor %>%
  ...
  theme(legend.position = "none", 
        axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x = expression(alpha*" level"), y ="Correlation with real line limits", title= "Edge removal rank correlation against real limits")
...
#                 ) 
#
#Yardstick version. Doesn't help much unless I have made the whole process using modelr
...
#the entropic degree function does not load properly find out why

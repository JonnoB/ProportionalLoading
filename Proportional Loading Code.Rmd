...
alpha_vector <- c(1.05, 1.1, 1.2, 1.5, 2, 3, 5, 7, 10, 15, 20, 50)[1:9]
setwd(Saved_Sims)
...
unique(predictions$Modelname) %>% map_df(~{
...
unique(predictions$Modelname) %>% walk(~{
...
SummaryofGC<- AttackRoundData %>%
  mutate(GridLoading = ifelse(Blackout==1, 0, GridLoading)) %>% #After redoing all the extract stats remove this line

...
Extracting stats is slow this extracts thhe stats and saves them as a file.
...
test <- AttackRoundData %>%
  filter(alpha=="alpha_value_105")
#lots of NAs why
table(is.na(test$GridLoading))

AttackRoundData %>% mutate(NAS = is.na(GridLoading)) %>%
group_by(alpha)
